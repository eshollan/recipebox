class RecipeIngredientsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @recipe_ingredient.save
        ...
        format.json { render json: @recipe_ingredient.errors, status: :unprocessable_content }
      end
    end
  end
  ...
  def update
    respond_to do |format|
      if @recipe_ingredient.update(recipe_ingredient_params)
        format.html { redirect_to @recipe_ingredient, notice: "Recipe ingredient was successfully updated.", status: :see_other }
  ...
  private
    ...
    def recipe_ingredient_params
      params.expect(recipe_ingredient: [ :ingr_qty ])
    end
end
